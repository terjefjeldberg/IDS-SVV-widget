(function () {
    ...
    function connectToStreamBim() {
        ...
        window.StreamBIM.connect({
            ...
            .catch(function (error) {
                ...
                setConnectionState(
                    "Ikke tilkoblet",
                    getErrorMessage(error) || "Kjor widgeten inne i StreamBIM for a hente modell-data",
                    "state-error"
                );
            });
    }
    ...
    function onIdsFileSelected(event) {
        ...
        readTextFile(file)
            ...
            .catch(function (error) {
                setRunStatus("Kunne ikke lese IDS-fil: " + getErrorMessage(error), "state-error");
            });
    }
    ...
    function loadSampleIds() {
        fetch(SAMPLE_IDS)
            ...
            .catch(function (error) {
                setRunStatus("Kunne ikke laste eksempel-IDS: " + getErrorMessage(error), "state-error");
            });
    }
    ...
    async function runValidation() {
        ...
        toggleBusy(true);
        setRunStatus("Henter modell-data fra StreamBIM og validerer mot IDS...", "");
    ...
    async function fetchModelDataFromStreamBim(api) {
        ...
        var rawObjects = coerceArray(objectsResponse.objects || objectsResponse.items || objectsResponse);
    ...
    async function invokeMethodGuessing(api, methodName, argsList) {
        ...
        throw new Error(methodName + " feilet. Forsokte argumentvarianter: " + errors.join(" | "));
    }
    ...
    function normalizeObjects(rawObjects) {
        ...
        coerceArray(rawObjects).forEach(function (item, index) {
            ...
            objects.push({
                id: pickFirst(item, ["id", "objectId", "dbId", "expressId"]) || "obj-" + index,
                guid: pickFirst(item, ["guid", "globalId", "ifcGuid", "GlobalId"]) || "",
                name:
                    pickFirst(item, ["name", "label", "title", "displayName", "Name"]) ||
                    pickFirst(item, ["guid", "globalId", "ifcGuid"]) ||
                    "Objekt " + (index + 1),
                type:
                    pickFirst(item, ["ifcClass", "type", "entity", "className", "category", "IfcClass"]) ||
                    "Ukjent type",
        ...
        Object.keys(container).forEach(function (key) {
            ...
            if (value && typeof value === "object") {
                if (hasPropertyIdentity(value)) {
                    ...
                    propertySets[key][value.name || value.baseName || key] = stringifyValue(
                        firstDefined([value.value, value.nominalValue, value.displayValue, value.Value])
                    );
                    return;
                }
    ...
    function normalizePropertyArray(items, fallbackSetName) {
        ...
        items.forEach(function (item) {
            ...
            if (item.properties || item.values || item.entries) {
                var setName =
                    item.name || item.propertySet || item.propertySetName || fallbackSetName || "Default";
                propertySets[setName] = flattenPropertyMap(item.properties || item.values || item.entries);
                return;
            }
            if (hasPropertyIdentity(item)) {
                var propertySetName =
                    item.propertySet || item.propertySetName || item.set || fallbackSetName || "Default";
                propertySets[propertySetName] = propertySets[propertySetName] || {};
                propertySets[propertySetName][item.name || item.baseName || item.key] = stringifyValue(
                    firstDefined([item.value, item.nominalValue, item.displayValue, item.Value])
                );
            }
        });
        return propertySets;
    ...
    function parseIds(xmlText) {
        ...
        var specEls = Array.prototype.slice.call(xml.getElementsByTagNameNS(IDS_NS, "specification"));
        if (!specEls.length) {
    ...
    function matchesAllRules(object, rules) {
        return rules.every(function (rule) {
            return evaluateRule(getPropertyValue(object, rule.propertySet, rule.baseName), rule).ok;
        });
    }

    function evaluateRule(actualValue, rule) {
        var hasValue = actualValue !== null && actualValue !== undefined && String(actualValue).trim() !== "";
    ...
    function matchValueRule(actualValue, valueRule) {
        ...
        var actual = actualValue === null || actualValue === undefined ? "" : String(actualValue);
    ...
    function getPropertyValue(object, propertySet, propertyName) {
        ...
        if (Object.prototype.hasOwnProperty.call(object.propertySets[propertySet], propertyName)) {
    ...
    function buildGroupDescription(rule, outcome) {
        ...
        if (outcome.reasonCode === "invalid-value" || outcome.reasonCode === "invalid-optional-value") {
            ...
            .map(function (group, index) {
                return [
                    ...
                    '      <div class="group-code">' +
                        ...
                        "</div>",
                    '      <p class="group-meta">' + escapeHtml(group.description) + "</p>",
                    "    </div>",
    ...
    async function createBcfForGroup(groupIndex) {
        ...
        try {
            ...
            setRunStatus('BCF opprettet for gruppen "' + group.title + '".', "state-ok");
        } catch (error) {
            setRunStatus("Kunne ikke opprette BCF: " + getErrorMessage(error), "state-error");
        }
    }
    ...
    async function createBcfForAllGroups() {
        ...
        for (var i = 0; i < state.validation.groups.length; i += 1) {
            try {
                ...
                setRunStatus(
                    "BCF stoppet etter " + created + " grupper. Siste feil: " + getErrorMessage(error),
                    "state-error"
                );
                return;
            }
    ...
    async function createBcfIssue(group) {
        ...
        return invokeMethodGuessing(api, methodName, [payload, { issue: payload }, { topic: payload }]);
    }
